...
// 信息解码模块：提取 rd、rs1、rs2，并根据指令类型输出唯一的立即数
class InformationDecoder extends Module {
  val io = IO(new Bundle {
    // 指令类型信号（来自 InstructionDecoder）
    ...
    val imm       = Output(UInt(32.W))  // 合并后的立即数
  })

  // rd 写使能（包含所有可能写回寄存器的指令）
  ...
  io.imm := PriorityMux(Seq(
    need_imm_i -> imm_i_sext,
    need_imm_s -> imm_s_sext,
    need_imm_b -> imm_b_sext,
    need_imm_u -> imm_u_raw,
    need_imm_j -> imm_j_sext,
    true.B     -> 0.U(32.W)   // 默认值
  ))
}

// 顶层 decoder 模块：输出指令识别信号、解码出的寄存器地址和立即数
class decoder extends Module {
  ...
  // 连接所有指令类型信号（复用 InstructionDecoder 的输出）
