...
int init_ram(const char *file_path) {
    ...
    if (!file_path || !*file_path) {  //路径不存在或文件为空
        log_err("RAM 文件路径为空");
        return -1;
    }
    ...
    fp = fopen(file_path, "rb");  //rb二进制只读模式
    check(fp, "打开 RAM 文件失败: %s", file_path);

    while(1){
        read_count = fread(&RAM[i], sizeof(int), 1, fp); //fp→ram  fread会自行维护一个文件内容指针，直到读不出来东西了
        if(read_count != 1){                             //读取异常或者读完了就退出
            break;
        }
    ...
    switch(opcode){
        case 115: {
            if(funct3 == 0) {
                ...
                // 保存显存到文件
                FILE *vram_fp = fopen("vram.bin", "wb");
                if (vram_fp) {
                    fwrite(VRAM, sizeof(int), 0x10000, vram_fp);
                    fclose(vram_fp);
                    printf("VRAM saved to vram.bin\n");
                }
