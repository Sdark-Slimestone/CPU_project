...
#define VGA_SIZE   (VGA_WIDTH * VGA_HEIGHT * 4)   // 480,000 字节
#define VGA_END    (VGA_BASE + VGA_SIZE)
#define VGA_WIDTH  400
#define VGA_HEIGHT 300
static int vga_framebuffer[VGA_WIDTH * VGA_HEIGHT] = {0};
...
void lui(int *gfr, int imm20, int rd){ //传入的imm20是被放在低20位的
    if(rd != 0){
        gfr[rd] = (unsigned int)imm20 << 12;//放回高20位
    }
    else{
        gfr[rd] = gfr[rd];
    }
...
void sw(int *ram, int *gfr, int rs2, int rs1, int imm12_merge){
    ...
    int addr = ((unsigned int)(gfr[rs1] + imm12_merge_signed)) >> 2;//程序保证对齐
    ram[addr] = gfr[rs2];
    gfr[PC] = gfr[PC] + 4;
}
...
void vga_show_framebuffer() {
    FILE *fp = fopen("framebuffer.ppm", "wb");
    if (!fp) {
        printf("无法创建 framebuffer.ppm 文件\n");
        return;
    }
    fprintf(fp, "P6\n%d %d\n255\n", VGA_WIDTH, VGA_HEIGHT);
    for (int i = 0; i < VGA_WIDTH * VGA_HEIGHT; i++) {
        unsigned char r = (vga_framebuffer[i] >> 16) & 0xFF;
        unsigned char g = (vga_framebuffer[i] >> 8) & 0xFF;
        unsigned char b = vga_framebuffer[i] & 0xFF;
        fwrite(&r, 1, 1, fp);
        fwrite(&g, 1, 1, fp);
        fwrite(&b, 1, 1, fp);
    }
    fclose(fp);
    printf("帧缓冲区已保存到 framebuffer.ppm\n");
}