...
unsigned int pmem_read(unsigned int addr) {
    ...
    if (offset + 3 >= MEM_SIZE) {
        printf("[ERROR] Out of bounds read at addr=0x%08x (offset=0x%x, MEM_SIZE=0x%x)\n", addr, offset, MEM_SIZE);
...
void pmem_write(unsigned int addr, unsigned int data, unsigned char mask) {
    ...
    unsigned int offset = base - MEM_BASE;
    if (offset + 3 >= MEM_SIZE) {
        printf("[ERROR] Out of bounds write at addr=0x%08x (offset=0x%x, MEM_SIZE=0x%x)\n", addr, offset, MEM_SIZE);
        simulation_finished = 1;
        good_trap = 0;
        return;
    }
...
static unsigned long long debug_cycle = 0;  // 调试用周期计数
static int ebreak_triggered = 0;            // ebreak 已触发标志
static int drain_cycles = 0;                // ebbreak 后排空剩余周期计数

// ebreak 回调，由 Verilog 模块通过 DPI-C 调用
void sim_finish(void) {
    if (!ebreak_triggered) {
        ebreak_triggered = 1;
        drain_cycles = 20;   // 排空20周期，让 UART 输出完
    }
}
