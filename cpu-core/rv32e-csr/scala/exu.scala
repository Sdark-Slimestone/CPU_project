...
class ALU extends Module {
  ...
  io.output := MuxCase(0.U(32.W), Seq(
    (io.alu_op === 0.U)  -> (io.source1 + io.source2),
    (io.alu_op === 1.U)  -> (io.source1 - io.source2),
    (io.alu_op === 2.U)  -> (io.source1 << shamt),
    (io.alu_op === 3.U)  -> (io.source1.asSInt < io.source2.asSInt).asUInt,
    (io.alu_op === 4.U)  -> (io.source1 < io.source2),
    (io.alu_op === 5.U)  -> (io.source1 ^ io.source2),
    (io.alu_op === 6.U)  -> (io.source1 >> shamt),
    (io.alu_op === 7.U)  -> (io.source1.asSInt >> shamt).asUInt,
    (io.alu_op === 8.U)  -> (io.source1 | io.source2),
    (io.alu_op === 9.U)  -> (io.source1 & io.source2),
    (io.alu_op === 10.U) -> io.source2
  ))
}
...
class InputMUXforALU extends Module {
  val io = IO(new Bundle {
    ...
    val inputfromIMM   = Input(UInt(32.W))
    val inputfromU     = Input(UInt(32.W))
...
class InputMUXforAGU extends Module {
  val io = IO(new Bundle {
    val inputfromRS1   = Input(UInt(32.W))
    val inputfromIMM_I = Input(UInt(32.W))
    val inputfromIMM_S = Input(UInt(32.W))
...
class BRU extends Module {
  ...
  val branch_cond = MuxCase(false.B, Seq(
    (io.branch_type === 0.U) -> eq,
    (io.branch_type === 1.U) -> !eq,
    (io.branch_type === 4.U) -> lt,
    (io.branch_type === 5.U) -> !lt,
    (io.branch_type === 6.U) -> ltu,
    (io.branch_type === 7.U) -> !ltu
  ))

  io.take_branch := Mux(io.jalr, true.B, branch_cond)

  io.target := Mux(io.jalr,
    (io.rs1_data + io.imm_i) & (~"b11".U(32.W)),
    io.current_pc + io.imm_b)
}
...
class EXU extends Module {
  ...
  val need_rs1_alu = use_rs1 && !io.is_sb && !io.is_sh && !io.is_sw
  ...
  aluMux.io.use_rs1      := need_rs1_alu && !io.is_auipc
  ...
  aguMux.io.inputfromIMM_I := io.imm_i
  aguMux.io.inputfromIMM_S := io.imm_s
