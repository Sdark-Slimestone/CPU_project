...
class LSU extends Module {
  ...
  val is_load = io.exu_to_lsu.op.lb || io.exu_to_lsu.op.lh || io.exu_to_lsu.op.lw ||
                io.exu_to_lsu.op.lbu || io.exu_to_lsu.op.lhu
  val is_store = io.exu_to_lsu.op.sb || io.exu_to_lsu.op.sh || io.exu_to_lsu.op.sw
  ...
  val shifted_wdata = MuxCase(wdata, Seq(
    (io.exu_to_lsu.op.sb) -> (wdata << (byte_offset << 3)),
    (io.exu_to_lsu.op.sh) -> (wdata << (byte_offset << 3))
  ))
  io.lsu_to_dmem.store_data := shifted_wdata

  // 写掩码生成
  io.lsu_to_dmem.mask := MuxCase(0.U(4.W), Seq(
    io.exu_to_lsu.op.sw -> "b1111".U(4.W),
    io.exu_to_lsu.op.sh -> Mux(byte_offset === 0.U, "b0011".U(4.W),
                             Mux(byte_offset === 2.U, "b1100".U(4.W), 0.U(4.W))),
    io.exu_to_lsu.op.sb -> (1.U(4.W) << byte_offset)
  ))
  ...
  val rdata = MuxCase(0.U(32.W), Seq(
    io.exu_to_lsu.op.lw   -> aligned,
    io.exu_to_lsu.op.lh   -> Cat(Fill(16, half_sel(15)), half_sel),
    io.exu_to_lsu.op.lhu  -> Cat(0.U(16.W), half_sel),
    io.exu_to_lsu.op.lb   -> Cat(Fill(24, byte_sel(7)), byte_sel),
    io.exu_to_lsu.op.lbu  -> Cat(0.U(24.W), byte_sel)
  ))
