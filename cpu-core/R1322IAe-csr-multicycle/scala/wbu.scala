...
class wbu extends Module {
  ...
  // 判断写请求是否有效（rd != 0）
  val valid1 = io.lsu_to_wbu_1.bits.rd =/= 0.U
  val valid2 = io.lsu_to_wbu_2.bits.rd =/= 0.U

  // 检测地址冲突（两个有效且 rd 相同）
  val conflict = valid1 && valid2 && (io.lsu_to_wbu_1.bits.rd === io.lsu_to_wbu_2.bits.rd)

  // 分配写端口
  io.wbu_to_grf.wr2.addr := Mux(valid2, io.lsu_to_wbu_2.bits.rd, 0.U)
  io.wbu_to_grf.wr2.data := io.lsu_to_wbu_2.bits.grf_wb_data

  io.wbu_to_grf.wr1.addr := Mux(valid1 && !conflict, io.lsu_to_wbu_1.bits.rd, 0.U)
  ...
  io.debug.rd1       := io.lsu_to_wbu_1.bits.rd
  io.debug.rd2       := io.lsu_to_wbu_2.bits.rd
