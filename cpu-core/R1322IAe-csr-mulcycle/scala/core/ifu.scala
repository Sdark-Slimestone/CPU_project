...
class IFU extends Module {
  val io = IO(new Bundle {
    // 【多周期适配】pc 更新使能: 多周期下每包完成(done)时更新一次, 其余拍保持
  ...
  // 【多周期适配】pc 更新追加 pc_en 门控(优先级结构与原版完全一致)
  pcReg := Mux(io.pc_en,
            Mux(io.csr_to_ifu.take_trap, io.csr_to_ifu.trap_pc,
            Mux(io.csr_to_ifu.take_mret, io.csr_to_ifu.mret_pc,
            Mux(io.exu_to_ifu.take_branch, io.exu_to_ifu.branch_target,
            Mux(io.idu_to_ifu.is_stall, pcPlus4, pcPlus8)))), pcReg)
